...
type Snapshot struct {
	...
	// The caller resolves each entry's cert/key (from the Origin cert registry)
	// before handing the snapshot over, so rendering needs no fallback logic.
	TLSModes map[string]TLSOverride
}

// TLSOverride is one host's panel-local TLS mode. Mode "cf_origin" pins a static
// Origin cert (CertPath/KeyPath, already resolved by the caller); any other value
// falls back to the on-demand behavior.
