...
// CADDY_HEALTH_INTERVAL (seconds) and CADDY_HEALTH_THRESHOLD (consecutive
// failures). "Our IPs" come from the shared OriginSet (origin_ips.go), the same set
// the Edge column uses — never from the panel domain's DNS, which is Cloudflare.
...
// judging only hosts that resolve to one of our origin IPs.
