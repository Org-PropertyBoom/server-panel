...
- **Status 2026-09-11:** the host-side pin is **applied and live**. A drop-in at `/etc/systemd/system/ppt-server-panel@root.service.d/origin-ips.conf` sets both `CADDY_HEALTH_SERVER_IPS` and `CUTOVER_ORIGIN_IPS` to all four addresses. The unit's `EnvironmentFile` (`/etc/ppt-server-panel/root.env`) sets neither name, so nothing overrides it, and both values were confirmed in the running panel's `/proc/<pid>/environ`. Four sites returned 200 after the panel restart, including launches.sg mid-renewal-window. The code fix is on the **unmerged** branch `fix/shared-origin-ips` (`e709cf3`; reviewed with two findings; Linux `make test` still pending). Once it ships the pin is redundant: delete the file, `daemon-reload`, restart the panel.
