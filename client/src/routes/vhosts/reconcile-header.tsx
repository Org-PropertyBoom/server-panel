...
export default function ReconcileHeader({
    ...
    return (
        <div className="border-b border-border bg-card/40">
            {state.protected && state.protected.length > 0 ? (
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-border bg-muted/30 px-6 py-2.5">
                    <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                        <Lock className="h-3.5 w-3.5" />
                        Protected
                    </span>
                    {state.protected.map((h) => (
                        <span
                            key={h}
                            className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-2.5 py-1 font-mono text-[11px] text-foreground"
                        >
                    ...
                    <span className="text-[11px] text-muted-foreground">
                        static in the main Caddyfile · never rendered/removed · every reconcile refuses a config missing these
                    </span>
                </div>
            ) : null}
