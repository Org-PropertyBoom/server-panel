...
- DEPLOYED + VERIFIED LIVE as `20260911035628` (another session had installed the `b0c6d7c` build `…034423` first; that build then showed the exact gap — it kept reporting "latest" for ~2 min after `…035628` published, since it had no refresh path). Timed on prod: plain poll 87→50 ms (cached); `?refresh=1` 31 s later **497 ms** (real GitHub check); immediate second refresh **56 ms** (paced, cached). Served bundle `index-GMwcfQue.js` contains the `refresh=1` call.
...
- **`services/caddy/health`:** `ReferenceDomains`/`PinnedIPs` removed → `OriginIPs func() map[string]bool` (read every cycle, so discovery is picked up). A host is judged only if an answer is one of our IPs; then EACH such IP is TLS-dialled directly with SNI = host (not `host:443` through DNS, which could land on a non-origin record). Failure = connect error, no cert, expired, or doesn't cover host; `lastError` = `TLS <ip>: <reason>`. Cloudflare / Elsewhere / NXDOMAIN / DNS-error hosts reset to 0 failures, no alert, no TLS dial. New `onOrigin` JSON field; `dnsOk` now means "resolved at all". Kept: read-only, skip the cycle when the origin set is empty/unparseable, `CADDY_HEALTH_THRESHOLD` debounce (default 2).
