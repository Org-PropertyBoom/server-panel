...
### 2026-09-11 - Review fix 2 on `fix/shared-origin-ips`: the Edge column classifies against the current origin set

- **Bug (review finding 2 on `e709cf3`):** `DNSProbeService` cached the finished Edge label for `edgeCacheTTL` (6h default, `CUTOVER_DNS_CACHE_HOURS`). An origin address added later (EC2 discovery runs every 30 min, or a config change) left its tenants labelled "Elsewhere" for up to 6h, while the probe, which reads `OriginSet()` every cycle, already judged them ours. That's the disagreement this branch set out to end.
- **Fix:** new `origin.EdgeCache` (`services/origin/edgecache.go`) caches the raw DNS answer (addresses, or the lookup error) and classifies it with `ClassifyEdge` against `Ours()` **on every read**, so there's no invalidation step. `DNSProbeService` is now a thin wrapper (`Lookup` = `net.DefaultResolver.LookupHost`, `Ours` = `originIPs`, `TTL` = `edgeCacheTTL`), and `Edges` converts to `EdgeInfo`. Unchanged: keys lower-cased and trimmed, blanks skipped, 16 workers, 5s per lookup, errors cached. Changed in passing: duplicate spellings of one host now cause one lookup instead of several. The Cutover pre-flight's `resolveEdge` was never cached and is untouched.
- **DECISION: cache the DNS answer, not the label.** The answer is the slow part; classification is a pure function, so re-running it on every read costs nothing and makes the Edge column and the probe agree by construction. Moved into the pure `origin` package, following this branch's own pattern for `ClassifyEdge`, so the tests run on Windows.
- **Tests (`services/origin/edgecache_test.go`):** `TestEdgeCacheReclassifiesWhenOriginSetGrows` (the finding: "Elsewhere" becomes "Origin" within the TTL once the IP joins the set, with no second lookup), `...ReusesAnswersWithinTTLAndRefreshesAfter`, `...CachesLookupErrors`, `...NormalisesAndDedupesHosts`.
- **Validation:** EXECUTED on Windows: `go test ./services/origin/ ./services/caddy/health/`, all passing. **Regression proof:** in a scratch copy with the old behaviour (origin set frozen when the answer is cached), `TestEdgeCacheReclassifiesWhenOriginSetGrows` FAILS with `edge="Elsewhere", want "Origin" (stale cached label)`. COMPILED ONLY: `GOOS=linux CGO_ENABLED=0 go vet ./services/... ./routes/...`, which confirms `dns_probe.go` builds. gofmt clean on LF copies. **Still needed before merge: a Linux `make test`.**
- **Left alone, noted:** a transient lookup error (timeout) is still cached for the full TTL and shows as NXDOMAIN, exactly as before this change. It's the same theme as review finding 1, but out of scope here.

### 2026-09-11 - Review fix on `fix/shared-origin-ips`: a DNS error no longer resets the probe's debounce

- **Bug (review finding 1 on `e709cf3`):** `realProbe` returned `onOrigin=false` on ANY lookup error, and `applyResult` then zeroed `Failures`, cleared `LastError` and dropped `Alert`. So one resolver hiccup silenced an active "Not reaching us" alert, and an origin host that was genuinely down never alerted if DNS timeouts alternated with TLS failures (fail → timeout → fail never reached threshold 2). `main` had the opposite bias: it counted DNS errors as failures.
- **Fix (`services/caddy/health/health.go`):** the lookup error is now split in two. A definitive **"no such host"** (`*net.DNSError` with `IsNotFound`) still means the host doesn't point at us, so the debounce resets. **Any other error** (timeout, SERVFAIL, a cancelled context, or anything that isn't a `*net.DNSError`) sets `probeResult.indeterminate`, and `applyResult` returns early, leaving failures, alert, last error and `checkedAtMs` exactly as they were.
- **DECISION: don't simply "never reset on DNS error".** That would leave a stale alert forever on a domain that has genuinely lapsed, which is the case being cleaned up right now. NXDOMAIN is a real observation that the host left; a timeout is no observation.
- **DECISION: leave `checkedAtMs` unchanged on an indeterminate cycle,** so "last checked" stays honest about the last real observation.
- **Tests (`health_test.go`):** `fakeNet` gains a per-host `dnsErr`. New: `TestDNSTimeoutDoesNotResetDebounce` (fail → timeout → fail alerts at threshold 2); `TestIndeterminateLookupKeepsActiveAlert` (table: timeout, SERVFAIL, `context.DeadlineExceeded`, and a plain non-`DNSError` error each leave an active alert's failures, last error and check time untouched); `TestNotFoundStillClearsAlert` (a lapsed domain's alert still clears). Existing `TestLeavingOriginResetsDebounce` and the NXDOMAIN tests unchanged and passing.
- **Validation:** EXECUTED on Windows: `go test ./services/caddy/health/ ./services/origin/`, all passing; the new tests were run by name with `-v`. COMPILED ONLY: `GOOS=linux CGO_ENABLED=0 go vet ./services/...`. gofmt clean on LF copies. **Still needed before merge: a Linux `make test`** (package `services` doesn't build on Windows). Review finding 2 (the Edge column's 6h cache vs discovered IPs) is not addressed here.
...
- **Flow actually run:** one stopped window (the Owner accepted up to 3 days of downtime), skipping the 2.6.2 intermediate restart. Prep while 2.6.2 served: remove `interval`/`burst`, set `admin localhost:2019`, adapt with `/tmp/caddy-new` into native-shape JSON, then check and validate. Then: live reconcile off → stop → cp → install → verify. My review of that plan added two things: turn live reconcile off before the stop (the panel persists `caddy.json` before reloading and is unreachable once Caddy is down), and the post-install diff to prove there's no adapter drift.
- 🔴 **My error:** I told the hub the official postinst only runs `try-restart` on upgrade, which "won't start a stopped unit". Caddy was already running before the Owner's `systemctl start`. The verbatim `dist/scripts/postinstall.sh` shows why: on **every** `configure`, `if deb-systemd-helper --quiet was-enabled caddy.service; then deb-systemd-helper enable …; deb-systemd-invoke start caddy.service`. So an enabled unit is started by the install. `try-restart` is a separate block. My earlier read came from a summarised fetch that filed the `was-enabled` block under "first installation", and I passed it on without seeing the condition. It held here only because the pre-install validate used a release binary whose module hash **matched the `.deb`'s** (`h1:XKxkMTgN…Wi0=`).
- **Lesson:** a summarised fetch of a shell script is not a verbatim read. For anything whose *conditions* matter, ask for the whole file verbatim, which I did only after it was proven wrong. The same trap as the B5 incident: behaviour I claimed "from source" was really from someone's summary of the source.
- Files: `docs/caddy-upgrade-2.11.md` (the postinst row and D6 corrected; an "Executed 2026-09-11" note with the flow run and next-upgrade guidance: pre-install validate with the exact `.deb` binary via `dpkg-deb -x` or a hash-matched release binary, and optionally a temporary `policy-rc.d` exiting 101, untested here). `docs/incident-2026-09-10-caddy-outage.md` (follow-up 1 marked DONE, with the results and the checks still pending).

### 2026-09-11 - One shared "our origin IPs" set; health probe only alerts on real origin TLS failures; Edge column gains ppt2

- **Goal:** fix the inverted reachability probe (it took "this server" from the panel domain's DNS, which is Cloudflare, so 85/96 hosts were flagged) and the Edge column missing ppt2 `52.77.202.62`, with one definition so the two can't disagree again. Code-only; nothing on the host touched.
- **New pure package `services/origin`:** `DefaultIPs` = `52.76.29.0, 52.76.123.15, 3.1.252.222, 52.77.202.62`. `Configured()` unions every set env of `PANEL_ORIGIN_IPS` (new canonical), `CADDY_HEALTH_SERVER_IPS`, `CUTOVER_ORIGIN_IPS` (old names honoured); any valid env value REPLACES the defaults, and all-garbage falls back to defaults. `Set` = configured ∪ discovered; `SetDiscovered` only replaces the discovered part, so metadata can never drop a configured address (other origin IPs may live on another box sharing storage). `IMDS` = IMDSv2 token → `macs/` → `<mac>/public-ipv4s` (404 per ENI skipped), 1.5s timeouts, `Proxy: nil`. The Edge classifier + Cloudflare ranges moved here (`ClassifyEdge`, `IsCloudflareIP`) so they're testable on Windows.
- **`services/origin_ips.go`:** process-wide `OriginSet()` (lazy, from env) + `StartOriginDiscovery(ctx)` (background, refresh every 30 min, silent on failure, `PANEL_ORIGIN_IMDS=0` disables). `main.go` (root mode) logs `origin IPs` with the list + source, then starts discovery before the probe. Boot never waits on IMDS.
- **`services/caddy/health`:** `ReferenceDomains`/`PinnedIPs` removed → `OriginIPs func() map[string]bool` (read every cycle, so discovery is picked up). A host is judged only if an answer is one of our IPs; then EACH such IP is TLS-dialled directly with SNI = host (not `host:443` through DNS, which could land on a non-origin record). Failure = connect error, no cert, expired, or doesn't cover host; `lastError` = `TLS <ip>: <reason>`. Cloudflare / Elsewhere / NXDOMAIN hosts reset to 0 failures, no alert, no TLS dial. A lookup that fails for any other reason (timeout, SERVFAIL, cancelled) leaves the previous status untouched; see the review-fix entry above. New `onOrigin` JSON field; `dnsOk` now means "resolved at all". Kept: read-only, skip the cycle when the origin set is empty/unparseable, `CADDY_HEALTH_THRESHOLD` debounce (default 2).
- **`services/dns_probe.go`:** `originIPs()` = `OriginSet().IPs()`; `resolveEdge` → `origin.ClassifyEdge` (same precedence as before: Cloudflare > Origin > Elsewhere). `services/health_probe.go`: wired to `OriginSet().IPs`; dead `splitCSV` removed.
- **Client:** `HostHealth.onOrigin?` + comments (`client/src/routes/vhosts/shared.tsx`). `UnreachableChip` and the header "Unreachable" count are unchanged and now only count real origin failures.
- **Validation:** EXECUTED on Windows: `go test ./services/origin/ ./services/caddy/health/`, all 20 pass (the four origins with good TLS not alerted; failing origin TLS alerts at threshold for all four reasons; Cloudflare/Elsewhere/v6-only/NXDOMAIN not alerted and never dialled; panel domain behind Cloudflare never resolved and not in "ours"; mixed answers dial only our IP; the Edge classifier puts all four in Origin; the metadata union keeps configured addresses; IMDSv2 flow via httptest; `realCheckTLS` against a loopback TLS server for ok / wrong host / expired / closed port). COMPILED ONLY: `GOOS=linux CGO_ENABLED=0 go vet ./services/... ./routes/...` OK, and the root package via `go vet -overlay` (placeholder for the git-ignored `client/build` embed) OK. `services/dns_probe_test.go` is compiled, not run (package `services` doesn't build on Windows). gofmt clean on LF copies. Client typecheck NOT run (`client/node_modules` not installed).
- **Host check before deploy:** if the panel unit sets `CUTOVER_ORIGIN_IPS` or `CADDY_HEALTH_SERVER_IPS` to a list without `52.77.202.62`, that override replaces the defaults and ppt2 stays missing. Check the startup log line `origin IPs … source=…`. The new code no longer needs the pinned mitigation.

### 2026-09-11 - Journal confirms the limiter panic; admin API open on :2019; health probe inverted (no code changed)

- **Journal confirmed the mechanism verbatim:** `http: panic serving 127.0.0.1:57036: invalid configuration: maxEvents = 0 and window != 0`, stack `net/http.(*conn).serve.func1` → `certmagic.(*RingBufferRateLimiter).SetMaxEvents` → `caddytls.(*TLS).Provision tls.go:184` → `adminLoad.handleLoad`. `conn.serve.func1` is net/http's handler-panic recovery, which is why PID 479641 survived. The hub's reconciliation and mine matched. The journal also shows `started background certificate maintenance` logged before the panic: the orphaned per-app cache goroutine (in v2.6.2, `t.certCache = certmagic.NewCache(...)` is per-app; there's no global cache and no `SetOptions`, so the running cache was untouched). `changeConfig`: the lock is released via `defer`; the raw config only updates after a successful run, so `GET /config/` reports the OLD running config.
- 🔴 **Security: the Caddy admin API is open on every interface.** The journal shows `address ":2019"`, `enforce_origin:false`, and `WARN admin endpoint on open interface; host checking disabled`. That's unauthenticated `POST /load`, i.e. full config replacement. Beyond the security group, it's **reachable from the Docker bridge**, so any compromised tenant container can reach it. **Fix, verified against v2.6.2 source + panel code before any edit:** `admin localhost:2019`, no `origins`, `enforce_origin` left off. The panel's `caddy reload` takes the address from the file's `admin.listen`; a loopback listen turns host checking on and allows `localhost`/`127.0.0.1`/`[::1]` on port 2019. The panel's `CurrentConfig` GET (`CADDY_ADMIN_URL`, default `http://localhost:2019`) sends no `Origin`, so enabling `enforce_origin` would 403 it and break the prior-config backup. On 2.6.2 this must ride a cold restart. Host check to make: is `CADDY_ADMIN_URL` overridden for the panel service?
- 🔴 **The health probe is inverted** (`services/health_probe.go`): its `ReferenceDomains` is `ProtectedHosts()`, i.e. the panel domain. `cp.propertyweb.co` resolves to **Cloudflare** (`104.21.69.82`, `172.67.206.132`), so every Origin tenant fails "resolves to this server". The only passes are Cloudflare hosts on the panel zone's IP pair, and their TLS completes at Cloudflare's edge, so they'd stay green through an origin outage. It reconciles exactly: 96 − 11 passing = 85, and that includes **all 31 Origin** hosts (tenant tab: Cloudflare 17 · Elsewhere 44 · NXDOMAIN 4 · Origin 31). The Edge column uses a different reference (`services/dns_probe.go` `originIPs()`), so the two disagree about what "here" is. Mitigation: pin `CADDY_HEALTH_SERVER_IPS=52.76.29.0,52.76.123.15,3.1.252.222,52.77.202.62` (all four origin addresses; each serves this Caddy's certificate, serial `05A8B5D6…0EEC`, tested from outside AWS). **The Edge column has the same blind spot:** `originIPs()` (`CUTOVER_ORIGIN_IPS`, default `52.76.29.0,52.76.123.15,3.1.252.222`) doesn't include ppt2 `52.77.202.62`, so ppt2 tenants such as `space-nova.com` and `spacenova.org` show as Elsewhere (its three are correct; it only misses ppt2, so the fix is to append `52.77.202.62`, not replace the list. An identical serial proves shared certificate storage and config, not strictly one instance). Code fix: spun off as a separate task.
- **My corrections:** (1) I told the hub "85 = the 2026-09-07 count of hosts whose DNS points elsewhere". That count came from this same probe, so it was the bug, not DNS. (2) The 2026-09-07 entry's "`apss.com.sg` does not resolve to this server" is wrong: it resolves to `52.76.123.15`. Also from the probe. (3) Earlier today I said "Force reloads are safe again" on a premise (a cold start) that never happened. The hub caught it; it's struck.
- **DNS, for the 3 October cohort:** LE's error is `rechecking caa: During secondary validation … networking error looking up CAA`. That's multi-perspective validation: LE's remote perspectives can't reach `ns1-3.webserver.sg`, which is why no local `dig` reproduced it. It covers **2 of 8** (`launches.sg`, `kevinfeng.sg`); the 6 on GoDaddy nameservers are still unexplained. Recorded in the incident doc.
- Files: `docs/incident-2026-09-10-caddy-outage.md` (the DNS update + a "Follow-on findings, 2026-09-11" section covering admin exposure, the hot-reload event, the inverted probe, and the 85 correction). No code changed.

### 2026-09-11 - 🔴 Runbook B5's Force reload failed on Caddy 2.6.2 (hot-reload limiter panic, recovered; process stayed up); B5 rewritten to a cold restart

- ⚠ **CORRECTED the same day:** the first version of this entry said the panic **killed the process** and `Restart=always` brought it back. **Wrong.** The host shows the same PID as yesterday and `NRestarts=0`; Caddy never died. The panic (if the journal confirms it) happened inside the admin `/load` HTTP handler, and **Go's `net/http` recovers handler panics**: it logs them and closes that one connection without a response. That's the EOF. So the process kept serving the **OLD** config (limiter included), while `publish()` had already persisted the **NEW** limiter-free `caddy.json` to disk. **Disk ≠ running, and every later hot reload (Force reload, live reconcile) fails the same way** until a cold restart, or until the pre-edit files are copied back (no restart needed, because running already equals pre-edit). The corrected reading of the 85 alerts is unchanged: the probe is pure DNS + TLS handshake, so it can't depend on the new config.
- **Shared-state check (hub question):** in Caddy v2.6.2 the certificate cache is **per-app** (`t.certCache = certmagic.NewCache(cacheOpts)`; no package-level cache, no `certCacheMu`, no `SetOptions`). The aborted provision built its own new cache, so the **running app's cache was untouched**. The leftover: certmagic v0.17.2 `NewCache` starts `go c.maintainAssets(0)`, and the panic skipped the Cleanup that stops it, leaving an **orphaned maintenance goroutine on an empty cache** (certificates load after the panic point, and managing them happens in Start, which was never reached). Every further hot-reload attempt leaks another. Harmless, but a reason to cold-restart soon. `changeConfig` also confirmed: the lock is released via `defer`, the raw config only updates after a successful run (so `GET /config/` reports the OLD running config), and contexts swap only after a full provision + start.

- **What happened:** following my runbook's B5, the Owner removed `interval 2m` / `burst 5` from `/etc/caddy/Caddyfile`, then did **Vhosts → Force reload**. Result: `reloaded: false`, `caddy reload … Post "http://:2019/load": EOF`, and the vhosts header showed **Unreachable 0 → 85**. Nothing was installed; still 2.6.2. **The step that triggered it was mine.**
- **ROOT CAUSE, from source (Caddy v2.6.2 + certmagic v0.17.2):** 2.6.2 keeps the on-demand limiter in a **package-level global** (`var onDemandRateLimiter`) that survives hot reloads. A hot `/load` of a config with no `rate_limit` runs `tls.go` Provision's else-branch, `SetMaxEvents(0)` and then `SetWindow(0)`, while the live limiter still has the 2m window. certmagic's `SetMaxEvents` does `if r.window != 0 && maxEvents == 0 { panic("invalid configuration: maxEvents = 0 and window != 0 …") }`. A panic inside `/load` kills the process, hence EOF, not an error response. **A cold start is safe:** a fresh process starts with window 0, so neither guard fires. **Only the transition from limiter to no limiter panics, and only when it's hot.** Going from no limiter to a limiter, or staying without one, doesn't panic. (It has to be confirmed in the host journal: expect the `maxEvents = 0` panic line.)
- **Q1 (what's on disk): the new, limiter-free `caddy.json`.** `ReloadOnly` → `backupPrior` → `publish()`, which persists first and reloads second (`engine.go:718` then `:723`); a persist failure aborts before the reload. The error came from the reload step, so the persist succeeded. `written: 0` counts rendered vhost `.caddy` files, not `caddy.json`. The backup `prior-20260911T020506Z.json` is the **live pre-edit config**, read from the admin API: a second valid 2.6.2 rollback source. Consequence: `Restart=always` (2026-09-10 drop-in) should have cold-started Caddy about 10s later **on the new config**, which is panic-safe. B5's goal was reached by accident.
- **Q2 (is the 85 current?): the probe can't see a ~10s crash.** `services/caddy/health` runs at start and then every 3 min (`CADDY_HEALTH_INTERVAL`), is **not** triggered by a reload, and alerts only after **2 consecutive failures** (`CADDY_HEALTH_THRESHOLD`, default 2), so it needs ≥ ~3 min of continuous failure. "Does not resolve to this server" counts as a failure. The reference IPs come from `ProtectedHosts()` (the panel domain) unless `CADDY_HEALTH_SERVER_IPS` is pinned. **85 = the 2026-09-07 count of hosts whose DNS points elsewhere.** A Caddy that stayed down would also fail the ~11 DNS-here hosts on TLS (≈96). Inference, not proof: these look like DNS-mismatch alerts, and the per-host reason settles it. Also, "0 → 85" is what a fresh in-memory prober looks like after two cycles.
- **Q3 (EOF on a successful load?): no.** EOF here is the panic.
- **Restore judgement given to the hub:** the Owner's restore (copy `/root/caddy-etc-pre-2.11/{Caddyfile,caddy.json}` back + `systemctl restart caddy`) is **safe**, because a cold start with the limiter is the known-good state. But check `systemctl status caddy` first. If it's active on the new config with the host count at 108 and tenants serving, **don't restore**. If you do restore, removing the limiter later must be done with a **cold restart**, never a panel reload.
- **Runbook fix (`docs/caddy-upgrade-2.11.md` B5):** the Force-reload step is gone. It now carries a 🔴 explanation of the panic, then: edit by hand → `caddy adapt` (2.6.2) into `/tmp/caddy-nolimit.json` → confirm no `rate_limit` → `validate` on **both** 2.6.2 and `/tmp/caddy` → `cp` into `/etc/caddy/caddy.json` → **`systemctl restart caddy`** (cold) → active + host count → only then is Force reload safe again. Fences re-checked: 96 total, indented blocks balanced 6/6.
- **Lesson, and what I missed:** I verified that removing `interval`/`burst` was *valid config* on both versions (adapt + validate), and that the limiter mattered for *behaviour*. I never checked what the running 2.6.2 does during the *transition* on a hot load. Validate and adapt run in a fresh process, so they **cannot** catch a bug that only exists in a long-lived process's global state. Config validity isn't the same as a safe live transition. For any future change that removes a Caddy feature on this host, check the live-transition path, or use a cold restart by default.

### 2026-09-11 - Validate caught `rate_limit`; when to remove the on-demand limiter on 2.6.2

- **Validate-first worked.** The Owner ran `/tmp/caddy-new validate --config /etc/caddy/caddy.json` (v2.11.4) and it failed: `tls: json: unknown field "rate_limit"`. This is the A3 prediction exactly: installing would have crash-looped under `Restart=always`. **Nothing was installed; 2.6.2 is still serving.** `.storage` is null and certs are under `/var/lib/caddy/.local/share/caddy/certificates/acme-v02…`, which matches the official package's path. There's **no staging dir**, so runbook A6 is corrected.
- Hub asked: is removing `interval`/`burst` safe on 2.6.2 right now? **Read from upstream source (Caddy v2.6.2, certmagic v0.17.2), not memory:**
  - `automation.go` `DecisionFunc`: `ask` first; on an allow it **falls through** to `onDemandRateLimiter.Allow()`, which returns "on-demand rate limit exceeded". So the limiter IS live here. `tls.go` Provision sets it from `rate_limit` burst/interval; nil disables it.
  - It's **one global limiter** for all hostnames.
  - certmagic v0.17.2 `renewDynamicCertificate` calls `checkIfCertShouldBeObtained`, which calls the `DecisionFunc`. So the limiter **gates handshake-time renewals**, the exact path in the panic trace.
- **Verdict: remove them, but as the first write of the migration window, then re-validate and install within minutes.** It's not a separate earlier change: the runbook's A2 said that, and it was wrong. Reasoning: the 8 October domains are **known** hosts, so `ask` allows them and the global limiter is the only Caddy-side throttle on their renewals. On 2.6.2, Let's Encrypt's 429 is the crash path. The hub's "ask makes it redundant" holds only for unknown hosts.
- **It cuts both ways:** on 2.6.2 a limiter denial during renewal drops the cert from the cache, same as an `ask` deny. So a burst can knock **healthy** hosts' certs out. Neither state is clean on 2.6.2; the fix is to minimise time there. Honest limit: its effect on the Let's Encrypt-facing attempt rate is unmeasured, because certmagic retries inside each job (`doWithRetry`) with its own backoff.
- Intent behind `interval`/`burst`: **not recorded anywhere in this repo** (the global block is hand-managed). The "ACME storm" was ended by `ask` refusing unknown hosts, not by this limiter.
- Files: `docs/caddy-upgrade-2.11.md`. A2 now defers to the new **B5** (by-hand edit after the B1/B4 backups, Force reload, A3 shows no `rate_limit`, straight into Phase C). A3 now expects `rate_limit` on this host. A6's staging note is fixed. The Phase C intro and the GO / NO-GO note cover the post-B5 state (if the install won't follow promptly, restore B1's Caddyfile and Force reload).
- Also answered: no panel compile-step change is needed. v2.11.4 `parseOptOnDemand` `case "ask"` builds a `PermissionByHTTP` into `PermissionRaw` with no deprecation log. Only the JSON `ask` field is deprecated, and the first panel reconcile on 2.11.4 rewrites it.

### 2026-09-11 - Caddy 2.6.2 -> 2.11.4 migration runbook (hub request; nothing run on the host)

- Trigger: the hub found the host runs **Ubuntu's `universe` package `caddy 2.6.2-14`** (dpkg-owned, dynamically linked, stripped; `Installed == Candidate`, so `apt upgrade` never moves it). The upgrade is a **source migration** to Caddy's official apt repo. They asked for a plan; timing and go/no-go are the Owner's.
- Files: `docs/caddy-upgrade-2.11.md` (new), a pre-flight/backup/validate/swap/verify/rollback runbook with one command per step, every write flagged, an expected output per step, and explicit STOP conditions. `docs/incident-2026-09-10-caddy-outage.md`: corrected claim + new "Answered 2026-09-11" section.
- **Verified from upstream source at tag v2.11.4, not from memory:**
  - The **panel emits Caddyfile, not JSON.** The JSON comes from a bare `caddy adapt` (`caddyctl/adapt.go:50`), so its shape follows the installed binary. The hub's framing ("your compiler must emit the new shape") didn't hold. The real risk is the **on-disk 2.6.2-era `caddy.json`** cold-started by 2.11.4 inside `apt install`'s restart (the postinst runs `try-restart` on upgrade).
  - A legacy JSON **`ask` is still honoured**: `tls.go` Provision converts it into `PermissionByHTTP`. It's deprecated, not silently dropped. Request shape unchanged: GET `?domain=`, 2xx = allow, so `/internal/tls-ask` and the loopback guard fit.
  - 🔴 **`on_demand.rate_limit` is gone** and module config is decoded with `StrictUnmarshalJSON` (`context.go` `LoadModuleByID`), so its presence would fail the cold start. Caddyfile `interval`/`burst` are **hard adapt errors**. Pre-flight A2/A3 check both.
  - **Official unit has no `Restart=`.** This corrects both the incident doc and the hub, who said upstream ships `on-abnormal`. `restart.conf` is the only restart policy. The official baseline `ExecReload` is `Caddyfile --force` again, so `reload.conf` must stay.
  - Official package user `caddy`, home `/var/lib/caddy`, no `Environment`, so storage is `/var/lib/caddy/.local/share/caddy`. Must be confirmed on host (A4-A7), or ~100 certs re-issue at once.
  - v2.11.4 pins **certmagic v0.25.3 / acmez v3.1.6**. The assertion that panicked is **checked** there. `renewDynamicCertificate` **re-consults permission** on renewal, which answers the open question for the target: the `2590ba9` eviction fix is sufficient there, and no cert purge is needed.
- **DECISION: validate-first is mandatory, not advisory.** The swap happens inside `apt install`, so Phase C (new binary in `/tmp`, `validate` today's `caddy.json` + `adapt` the Caddyfile + host-count parity) is the only point where a failure costs nothing.
- **DECISION: rollback restores the pre-migration `caddy.json` BEFORE downgrading.** After the first panel reconcile on 2.11.4, `caddy.json` uses `permission`, which 2.6.2's strict decoder rejects, so a naive downgrade crash-loops. Rollback also saves the 2.6.2 `.deb` locally (B4) so it doesn't depend on the Ubuntu archive.
- **DECISION: `--force-confold` on every install.** `/etc/caddy/Caddyfile` is a dpkg conffile; the maintainer's version would replace the hand-managed file. **D2 uses `curl -o`, not `tee`**, because yesterday every paste landed twice and a doubled `tee` appends.
- ⚠ New exposure recorded in both docs (read from source, NOT tested): on 2.11.4 a **panel outage** also fails handshakes for on-demand certs in their renewal window (permission denied, so the cert is dropped from the cache), not only uncached hosts. Panel-down alerting is TLS alerting.
- NOT done: nothing run on the host; no config touched. The target version must be re-confirmed by `apt-cache policy` on the day (D5 stops on any mismatch).

### 2026-09-10 - TLS-ask allowlist: evict on every panel-owned revocation (from a hub bug report)

- Trigger: the design-templates hub reported "a cached ALLOW beats an explicit DENY" in `CertAllowed`. **Verified before acting — the headline was WRONG, the second half was right, and the suggested fix did not address the reporter's own strongest case.** Corrections sent back.
- ❌ **WRONG (theirs):** "disabling a host in the panel has no effect until the 12h TTL". `SuppressHost` already calls `certAskEvict` immediately, memory + disk (`caddy_engine.go:1100`, helper `:532`). Stronger: a suppressed host **cannot** be re-cached — `certAskStore` is only reached after a cache MISS *and* the suppression check *and* a DB allow, so the flagged `:457`/`:461` ordering was safe by construction. They read the ordering but not the eviction path. Told them to correct the Owner, who may now distrust a control that works.
- ✅ **RIGHT and worse than framed:** `website_hosts` is stack-owned + read-only here (`caddy/db/write.go:13`, `routes/post/vhost/main.go:433`), so the panel never sees that delete and **their preferred "invalidate on delete" is not implementable for that table.** The 12h TTL really is the only lever, which makes the `TLS_ASK_CACHE_HOURS` workaround they gave the Owner the correct answer, not a stopgap. Corollary: their reorder fixes nothing for a *deleted* host, which is never in `SuppressedHosts` at all.
- 🔴 **THE REAL BUG THEY MISSED — panel-OWNED revocations never evicted.** `HostCertAllowed` honours `is_active = 1 AND deleted_at IS NULL` (`caddy/db/db.go:161`) so the DB refuses, but the cache answered first for up to 12h. Four paths fixed: `DeleteSystemHost` (:1060), `DeleteRedirect` (:1069), `SaveSystemHost`/`SaveRedirect` **rename** (the old name was orphaned — headers and TLS mode were already dropped, the allowlist entry was not), and **deactivate** (`is_active=0`).
- Files: `services/caddy_engine.go` — `certAskEvictHost` helper (empty-guard + same key normalization as the allowlist), eviction wired into the four paths, `SuppressedHosts()` moved above the cache in `CertAllowed`. `services/caddy/db/write.go` — new `RedirectByID` mirroring `SystemHostByID`. `services/caddy_engine_certask_test.go` (new, 5 tests).
- **DECISION — evict AFTER the write succeeds, never before.** If the delete/update fails the row still serves, and a pre-emptive evict would deny a LIVE host a cert. A failed id->host lookup is likewise non-fatal: the caller asked for a delete, and a missed eviction degrades to the old TTL behavior rather than failing the operation.
- **DECISION — deliberately NOT wired into `PinRoute`** (`:958`), even though it calls `conn.DeleteSystemHost`. That path converts a row into a hand-written static block: the host **keeps serving**, so evicting there would break a live host. It calls the DB method directly, not the engine wrapper, so the new eviction correctly does not reach it.
- **DECISION — the reorder is belt-and-braces only.** It costs one small local-SQLite read on the ask fast path; errors fall through instead of denying, so it cannot fail the platform closed. Kept because the doc comment's whole promise is "staleness is handled by explicit invalidation", and that promise should not depend on every future writer remembering to evict.
- ⚠ **VALIDATION LIMIT — the tests were NOT executed.** `GOOS=linux CGO_ENABLED=0 go vet ./services/...` is clean and gofmt is clean (checked on LF copies — every file in this repo is CRLF so `gofmt -l` lists all of them and is useless as a signal). But this is a Windows box with **no C toolchain**, so go-sqlite3 compiles to its stub, and package `services` does not build here at all (`system.go:269` `syscall.Statfs_t`). I confirmed the tests reach `NewSettingsService` before hitting the sqlite stub, so the wiring is right — **they still need a real `make test` run on Linux.**
- Open, host-side, unchanged: upgrade Caddy off 2.6.2; the 3 October cohort; alerting. And their unanswered question — does certmagic re-consult the ask on RENEWAL? Not answerable from this tree (no certmagic in go.mod); settle with `go version -m /usr/bin/caddy | grep -i certmagic` on the host.

### 2026-09-10 - Take custody of the 2026-09-10 Caddy outage write-up + verify the reload path

- Handover from the design-templates hub session: the shared Caddy died **2026-09-10 00:06:14 UTC and stayed dead 5h45m**, taking every tenant AND `cp.propertyweb.co` with it. Chain: LE could not resolve DNS for 8 tenant domains -> both challenges failed -> 5 failed authz/hour -> **HTTP 429** -> `acmez` in **Caddy 2.6.2** type-asserts a nil authorization and **panics** -> **no `Restart=` policy on this host** -> no alerting -> found by a person opening a browser.
- Files: `docs/incident-2026-09-10-caddy-outage.md` (new) — copied out of `design-templates`, which was only ever a temporary holder; the HELD IN TRUST header is replaced with a custody note. The hub session deletes its copy on confirmation. I did **not** touch that repo.
- 🟢 **CHECKED THE FLAGGED TRAP — this repo was never armed.** The hub warned that `ExecReload` had been aimed at `/etc/caddy/Caddyfile --force` while `ExecStart` ran `caddy.json`, so any `systemctl reload caddy` would have forcibly replaced the running config for ~100 domains. **Nothing here calls it.** The only `systemctl` in Go is read-only `systemctl is-active --quiet` (`services/apps.go:131`); reloads shell the binary directly as `caddy reload --config <compiled path>` (`caddyctl/compile.go` `ReloadFromFile`), naming the config explicitly and bypassing systemd's `ExecReload` entirely; `adapt.go:10` already records "NEVER `systemctl reload`" as a rule. `public/install.sh:153` is `systemctl enable --now caddy` — a start via `ExecStart`, not a reload. The host-side fix was still right: it closed the trap for a hand-typed command.
- CONFIRMED structural finding #1 for the hub: **`caddydash.service` does not exist on the host, and there is no gap** — this panel writes `/etc/caddy/caddy.json` (`services/caddy/`, commit `9c5229a`). They inferred it; the code confirms it.
- **`docs/tls-scaling.md` (2026-07-26) had already named both mechanisms** and the hub had not read it. It lists "5 failed validations per hostname per hour (**the dead-domain case**)" and warns LE limits bite on **bursts**; and it already says "Do not put the panel's own domain on on-demand … keep `cp.propertyweb.co` a static/pinned block", recording a prior event where a panel restart took cp/go3/laravel3 to curl `000` at once. So the bootstrap finding is a **recurring documented class here**, not new. Cross-referenced both into the incident doc.
- ⚠ **NOT DONE — host-side, needs the Owner.** 🔴 **3 October 2026**: 8 domains co-issued 5 July all expire within 4h of each other and are all failing renewal now. My read: the durable fix is to **stagger the re-issue**, not just renew them — `tls-scaling.md`'s stagger advice covers onboarding, and nothing today breaks up a renewal cohort a past bulk onboarding created. Hub's priority order (I agree): upgrade Caddy off 2.6.2 first (it is what turns an ACME error into a dead process), then the cohort, then alerting (port-443 + cert-expiry checks would each have caught this independently).
...
- 🔴 CAUSE (2026-09-07 full-TLS outage): `caddy.service` runs `User=caddy`, which cannot read the root-only `/home/server/.caddy` import. While the panel only POSTed adapted JSON to the admin API the good config lived **only in Caddy's memory** — a cold start re-read the Caddyfile as `caddy`, the import resolved to nothing, every tenant vhost dropped (CF-proxied 525, direct ERR_SSL_PROTOCOL_ERROR). Recovered via panel Force reload. The host now boots from a compiled, caddy-readable `/etc/caddy/caddy.json`; this work keeps that file current and authoritative.
- Files: `services/caddy/caddyctl/compile.go` (new) — `PersistConfig` + `ReloadFromFile`; `services/caddy/config/config.go` — `CompiledConfigPath` (default `/etc/caddy/caddy.json`, env `CADDY_COMPILED_CONFIG`); `services/caddy/reconcile/engine.go` — `ConfigPersister` interface, `publish()`, `Result.CompiledPath`, both call sites. Tests: `caddyctl/compile_test.go`, `reconcile/compile_test.go`.
- Per-ID: 1 write `.caddy` files **no-change** · 2 adapt as root **no-change** (already shelled as the root process) · 3 validate/abort **fixed** (explicit `json.Valid()` gate before anything is written or applied) · 4 atomic write **fixed** · 5 reload from the file **fixed** · 6 keep prior backup **pass**.
- **Atomic write**: temp in the SAME directory (a cross-filesystem rename silently degrades to a non-atomic copy) → fsync → chmod 0644 → chown `root:caddy` → `rename()` → **fsync the DIRECTORY** so the rename itself survives power loss. chown is best-effort BY DESIGN: 0644 is already caddy-readable, and refusing to publish a valid TLS config over a cosmetic ownership failure would trade working TLS for tidiness.
- **DECISION — persist BEFORE reload.** A failed reload therefore leaves `caddy.json` NEWER than the running config. That asymmetry is the safe side: the next cold start lands on the intended config. Reload-first would leave a window where the running config exists nowhere on disk — the outage's exact shape.
- **DECISION — Force reload recompiles too**, not only Live reconcile. Force reload is the tool that RECOVERED the site during the outage, so it is the one that most needs to write the file; repairing the running config while leaving the boot config stale is how this returns on the next restart.
- **DECISION — `ConfigPersister` is an OPTIONAL type-asserted interface**, not new methods on `Reloader`, so existing fakes and any read-only/off-host engine keep working unchanged. `publish()` is the ONLY path that applies a config, so "never apply what you did not persist" cannot be bypassed. `CADDY_COMPILED_CONFIG=""` falls back to the old admin-API Load — the only remaining way to do a memory-only apply, off-host only.
- Tests (12, and **these packages RUN on Windows so they were executed, not just compiled**): caddyctl — writes-and-leaves-no-temp, mode 0644 (skipped on Windows), replaces-existing-completely (proves `rename()` swaps the whole inode; an in-place write would leave a longer prior config's tail and produce invalid JSON), rejects invalid/short (4 subtests) + asserts NO file is created on reject, rejects empty path. reconcile — compiles-then-reloads-from-file (asserts persist→reload ORDER and that admin `Load` is never called on the compiled path), force-reload-also-recompiles, persist-failure-aborts-before-reload, reload-failure-is-reported-but-file-persists, falls-back-when-compiling-disabled, publish-rejects-non-JSON, compiled-path-not-created-on-adapt-failure.
- VERIFIED LIVE ON PROD (Owner ran the two host-side commands): **V1** `POST /post/vhost/reload` → `reloaded:true`, `compiled_path:/etc/caddy/caddy.json`, backup `prior-20260907T041753Z.json`, 79ms; mtime `03:40:05 → 04:17:53`; no `.tmp` strays. **V2** after `systemctl restart caddy`, `curl localhost:2019/config/ | jq '[…host[]?]|unique|length'` → **108** (107 rendered files + the static `cp.propertyweb.co` block); panel back through Caddy on the first poll. **V3** `openssl s_client -servername apss.com.sg` cert IDENTICAL before/after (`CN=apss.com.sg`, Sep 3 → Dec 2 2026). **V4** 107 files still in `/home/server/.caddy`. **V5** unit-tested.
- STRONGEST SIGNAL: the compiled file is **132798 bytes both before and after** the panel wrote it — byte-identical to the Owner's hand-migrated `caddy.json`. The panel reproduces the migrated config exactly, not a variant of it.
- ⚠ HONEST LIMIT ON WHAT THIS PROVES: the Owner's manual migration had already placed a good `caddy.json` at 03:40, so the 2026-09-07 cold start would have succeeded regardless. What is proven is that the file now STAYS current automatically. **The real regression test is the next live reconcile with non-zero writes — `caddy.json` must move again on its own.** `liveReload` is on, state `in_sync` 107/107.
- REPORTED TO HUB: the **V3 check as written cannot work** — `apss.com.sg` does not resolve to this server (52.76.123.15); 85 of 96 monitored hosts are in that state (DNS at Cloudflare/elsewhere). It only tested via direct SNI against `127.0.0.1:443`. Pre-existing and unrelated to this change, but the checklist wording needs fixing.
- DID NOT: touch the admin API port/security, move the vhost source, touch `/var/lib/caddy`, or run any reload as the `caddy` user. Did not restart Caddy — that stayed the Owner's.

### 2026-08-13 - Panel fixes: CPU metric, not-deployed rows, restart latch, file tree

- Four unrelated defects found by the Owner using the panel; grouped here because they shipped together. Commits `f089949`, `1ff5f58`, `f210fec`, `3abc3b0`, `bc5727c`, `d4c4ed3`.
- **CPU metric was measuring itself** (`services/system.go`). Dashboard read 36.8% while all 7 containers summed to 0.3%. Three defects: (a) **steal counted as our load** — `(total-idle)/total` with steal inside total and outside idle, so hypervisor-stolen time read as a busy server; (b) `guest`/`guest_nice` **added to the total** although the kernel already includes them in `user`/`nice`, inflating the denominator; (c) the **100ms inline sample** was ~10 jiffies per core, so on 2 cores a SINGLE busy jiffy moved the reading ~5% — and every request paid 100ms for it. Now differences against the previous poll (~8s window, no wall-clock cost), excludes steal, and reports `steal`/`iowait` separately. **Live result after deploy: usage ~1%, steal 0% — the server was idle the whole time.**
- **Self-check found a bug in that fix**: `busy := totalDelta - (now.idle-prev.idle) - (now.steal-prev.steal)` is all `uint64`; a counter going backwards (CPU hotplug, reset) wraps to ~1.8e19 rather than going negative, rendering garbage. Clamped via `sub()` + `pct()` bounded 0..100. My first regression fixture was also wrong — it made `after.total` smaller than `before.total`, which underflows inside the test and models a case `cpuStatus` never reaches.
- **NOT DEPLOYED rows showed only a name** (`services/container_compose.go`). `parseComposeServices` returned service NAMES only, so synthetic rows had nothing to populate Image/Ports with. Now returns `composeService{Name,Image,Build,Ports}` — quoted images and trailing comments stripped, block-form `build:` recorded as `(build)`, ports tracking ends at the next property key so a following `volumes:` list cannot leak in. ⚠ The signature change **silently broke the existing test while it still COMPILED** — `reflect.DeepEqual` takes `interface{}`, so `[]composeService` vs `[]string` only fails at runtime. Caught by reading the test, not the compiler.
- **Update dialog latched on forever** (`client/src/_layouts/_components/header.tsx`). `setRestarting(false)` sat AFTER `await waitForServer()`, and `finally` only cleared `updating` — so a reconnect timeout threw straight past it and left `restarting` true for the life of the page: dialog stuck, all buttons disabled, beforeunload guard armed, with no way out but the manual reload the dialog tells you not to do. Fixed by clearing in `finally`; budget raised 90s → 5min (the update DOWNLOADS a binary, so the wait is bounded by transfer, not process start); added an elapsed-seconds counter so "working" is distinguishable from "hung". Confirmed live that `/healthz` and `/post/system` were answering while the dialog was still hung.
- **File tree felt misaligned** (`client/src/routes/files/index.tsx`). Measured the live DOM first: sibling folder and file rows already had icons at the same x and names at the same x — the geometry was correct. The real difference was **glyph count**: folders drew chevron + folder icon, files drew a blank spacer + file icon, so a folder's ink began 16px earlier. VS Code draws ONE glyph per row and no folder icon. Removed the folder icon (adding a file icon would NOT have fixed it), indent 10 → 16px (it was narrower than the twistie slot itself), added indent guides. Verified live at three depths: glyph and name x identical for folder vs file (`92/114`, `124/146`, `156/178`). Also dropped 7 pre-existing dead lucide imports — invisible because tsconfig sets no `noUnusedLocals` and the bundler tree-shakes them.
- Validation: `GOOS=linux go build ./...` 0, vet clean; `tsc --noEmit` 0; `npm run build` OK. `services/` cannot RUN tests on Windows (`syscall.Statfs`), so the pure CPU/compose functions were extracted verbatim into a scratch module and executed there — all pass; `make test` on host remains the real gate. All four verified live on the deployed panel.

### 2026-07-29 - New container flow: compose-file output + loopback-default ports

- Spec: design-templates `designs/shared/panel-new-container.md` @ **301c1ad**, which SUPERSEDES de67554 (Owner changed the output format after reading it). Earlier commit ff44d82 built the de67554 version (`docker run` + Review showing a synthesized command); this pass reworks the output entirely.
- **P0 §1 loopback default**: publish-scope radio; `127.0.0.1` (default) → `ports: ["127.0.0.1:9001:8080"]`, `0.0.0.0` → `ports: ["9001:8080"]` behind a named inline confirmation. Lines with their own bind address pass through untouched. Not remembered between uses — exposure is chosen each time. This is the fix for the class of bug that put MinIO/openinary/mysql:3306 on the public internet.
- **P0 §2 compose output**: `PlanContainer` now GENERATES the YAML; `CreateContainer` writes `/home/server/containers/<name>/docker-compose.yml` then runs `docker compose up -d` in that dir. No `docker run` anywhere. 🔴 Hard guard: `IsManagedComposeDir` confines every write to the managed root, re-checked in CreateContainer even though PlanContainer built the path (defence in depth) — a stack repo's compose file can never be written into/near/over. Managed vs unmanaged is visible: `Container.Managed` is true only when the working dir is under the managed root; nothing is retro-generated for the rest. Managed root added to the compose-discovery scan so panel-created containers appear as projects.
- **P1**: §3 network bridge/host/none (host/none disable Ports + explain; the combination also blocks server-side rather than surfacing as a cryptic Docker error) · §4 Environment defaults to From file → `env_file: [<path>]`, path referenced never inlined, 0600 checked · §5 guards (duplicate name blocks, docker.sock blocks unless confirmed, `/`,`/etc`,`/root`,`/home/server/.caddy` warn, `:latest` note). §6/§7 (P2) NOT attempted.
- SPEC TENSIONS REPORTED (in the commit body, not worked around): (a) the port validator `^(\d{1,5}:)?\d{1,5}...$` REJECTED `127.0.0.1:9001:8080`, so the safe default was unrepresentable — widened to allow an optional bind address; (b) §5's "Name empty: allowed, Docker auto-names" doesn't hold once the output is compose, because the name IS the directory — a name is derived from the image and shown at Review, which meets the stated intent.
- Conservation: the "For stack apps, use their deploy pipeline instead" warning is verbatim and above the fold; only `docker run -d` → `docker compose up -d` was reworded, which the spec explicitly permits since the implementation no longer uses docker run. Restart policy + `unless-stopped`, `src:dst[:ro]`, `host:container`, the container list and all other routes untouched.
- Tests (`container_plan_test.go`): loopback pass-through, bare-port PUBLIC warning, explicit `0.0.0.0` AND LAN binds still public (a "has a bind ⇒ private" shortcut would have missed both — caught by running the logic standalone), duplicate blocks + enforced in CreateContainer, docker.sock, host-networking, GatewayService expressible, write-confinement + traversal name rejected, env path-not-values, YAML port quoting, derived name, `:latest`.
- Validation: `GOOS=linux go build ./...` 0; gofmt clean; vet clean; services tests COMPILE for linux (cannot RUN on Windows — `make test` on host); client tsc 0; `npm run build` OK. NOT deployed; `docker inspect` + the "nothing written in stack repos" check need the host.
...
- MODEL: register N Origin certs (a LIST, no "default"); on switching a host to cf_origin the panel SELECTS the cert whose SANs cover that hostname — reusing the `x509.VerifyHostname` from the c204d8f guard. Removes two concepts instead of adding one.
- Backend: new `origin_certs` table (cert_path PK, key_path) + **auto-migration** — the legacy `vhost_origin_cert_path`/`_key_path` settings pair is INSERTed as the first registry entry, so grafana/media (live on the propertyweb cert) are unaffected. `OriginCerts()` returns entries with `Covers` (SANs, or CN) + `Expires` parsed from the PEM at read time; `AddOriginCert` validates the PEM parses + key exists at REGISTRATION (typo caught early, not when a host breaks); `SelectOriginCertFor(host)` picks the covering cert — **exact SAN match beats wildcard, else registration order** (deterministic, no coin-flip), and errors with "no registered Origin cert covers X (registered certs cover: …)". `verifyOriginCertCoversHost` kept for the explicit per-host escape hatch. `resolveTLSModeCerts` fills each cf_origin host's cert/key from the registry when building the snapshot, so **`Snapshot.DefaultOriginCert/Key` are GONE** and plan.go has no fallback logic — it just skips (with a reason) a host whose cert didn't resolve.
- Routes: `POST /post/vhost/origin-cert` (register) + `DELETE ?cert=` (unregister); both return the registry with coverage. State: `originCerts[]` replaces `originCert`/`originKey`.
- UI: "Origin cert" → **"Origin certs (N)"** registry modal — list showing each cert's basename, what it covers, expiry, full path, parse errors, and a remove button; plus a register form. TLS modal now shows **"Will use origin-ca-propertyboom.pem — covers *.propertyboom.co, propertyboom.co"** (auto-selected) instead of asking which file to type; the per-host path fields are behind an "advanced" toggle as the escape hatch. TLS column shows the auto-selected cert's basename with "(auto-selected — covers …)" on hover. Client-side `originCertFor()` mirrors the backend selection for preview only (backend decides).
- Validation: `GOOS=linux go build ./...` 0; gofmt clean; render+reconcile tests pass; client tsc 0; `npm run build` OK.
- ✅ RESOLVED 2026-07-27: the SPOF acceptance test PASSED — 60 unbroken 200s across a live `systemctl restart ppt-server-panel@root`. Blocker cleared; d9e71ae works. Test gotchas (cost several attempts, now in `docs/tls-scaling.md`): run from REAL SSH/EC2 Instance Connect (the panel's own web terminal is a child of the panel and dies with the restart); `sudo -i` first or systemctl hits a polkit prompt and times out; a Proxied host 403s bare curl via Cloudflare bot protection, so send a FULL browser UA — and note a 403 still proves TLS succeeded, `000` is the failure signal.

### 2026-07-26 - FIX: tls-ask was a platform-wide TLS SPOF (persist + long-cache the allowlist)

- INCIDENT (hub-reported, must ship BEFORE the next deploy): Owner clicked "Check Update" → panel restarted → `/internal/tls-ask` unreachable → fail-closed → **go3, laravel3 AND cp.propertyweb.co all curl 000 simultaneously**. Self-healed when the panel returned, but a ROUTINE PANEL DEPLOY = platform-wide HTTPS outage, and deploying the queued commits would have repeated it.
- ROOT CAUSE — corrects an assumption in BOTH the hub spec and my own code comments ("existing certs keep serving; only NEW issuance pauses"): **Caddy consults `ask` on HANDSHAKES** for any hostname not in its in-memory cert cache, not just at issuance. Fail-closed + a 45s positive cache ⇒ any panel restart >45s drops TLS for every on-demand host. The panel was a single point of failure for platform TLS.
- FIX: (1) TTL 45s → **12h** (`certAskCacheTTL()`, override `TLS_ASK_CACHE_HOURS`). (2) **Persisted allowlist** — new `tls_ask_allow` SQLite table + `TLSAskAllowlist`/`SetTLSAskAllowed`/`DeleteTLSAskAllowed`; loaded into memory lazily via `sync.Once` on first ask (fast, failure-free boot), written on every positive answer. (3) **Cache consulted BEFORE the DB**, so a recently-valid host answers 200 while the panel is warming or the shared MySQL is unreachable; only a MISS takes the DB path and only that is fail-closed. (4) Staleness via **explicit eviction** not a short clock — `certAskEvict` on suppress/disable so disabling stops issuance immediately. (5) Genuinely UNKNOWN hosts stay fail-closed (the abuse guard that killed the ACME storm).
- RESIDUAL RISK (documented, not fixable from inside the panel): while the process is fully DOWN the ask is unreachable and Caddy refuses for hosts not in its own cert cache. Mitigations: short restarts; use cf_origin/static certs for hosts that must not depend on the panel. **Never put the panel's own domain on on-demand** — circular dependency (panel down → its domain can't get a cert → can't reach the panel to fix it); keep cp.propertyweb.co a static/pinned block. Recorded in `docs/tls-scaling.md` (new section "The ask endpoint is on the HANDSHAKE path").
- ACCEPTANCE TEST (must run ON THE HOST, can't be done from dev): restart the panel while curling a live on-demand host in a loop — it should stay 200 throughout. `while true; do curl -s -o /dev/null -w "%{http_code} " https://go3.propertyweb.co; sleep 1; done` then `systemctl restart ppt-server-panel@root`.
- Validation: `GOOS=linux go build ./...` 0; gofmt clean; services test compiles for linux.
...
- **Item 1 — the ask endpoint (the panel's core job, INERT until Caddyfile points at it):** `GET /internal/tls-ask?domain=<h>` → 200 authorize / any non-200 refuse. `services/caddy/db/db.go` `HostCertAllowed(ctx,host)` — three cheap lookups (platform_hosts + platform_redirect_hosts active&!deleted; website_hosts existence — it's lean, no is_active). Match is CASE-INSENSITIVE (`LOWER(host)` vs a lowercased param) so a stack-written website_hosts row with any casing still authorizes — the panel can't normalize that stack-owned table, so we never deny a live host a cert on a casing quirk (Owner asked to code-guard it, 2026-07-26). Forgoes the host index, but the tables are small + cached + Caddy rate-limits the ask, so cost is trivial. `caddy_engine.go` `CertAllowed(ctx,host)`: protected/panel domains always allowed; suppressed ("Disabled · edge") refused; else DB lookup. **FAIL-CLOSED** — any DB error → (false,err) → non-200 (existing certs keep serving from Caddy storage, only NEW issuance pauses). NOTE: deliberately OPPOSITE of the /dashboard gate's fail-OPEN ruling — different context. Positive answers cached 45s (map+mutex, self-evicting); negatives/errors NOT cached so enable→visit works. Handler `routes/post/tlsask/main.go`; route `GET /internal/tls-ask` behind NEW `loopbackOnly` guard (RemoteAddr loopback AND Host header loopback → only Caddy's direct 127.0.0.1 call passes; a Caddy-proxied public request carries the public Host and is refused) + rootOnly. Included redirects in authz (item 3 adds on_demand to redirect blocks too → they need certs).
- **Item 3 — vhost template (BUILT, GATED, default OFF):** `render.Host.OnDemandTLS` emits `    tls {\n        on_demand\n    }\n`; **skipped for wildcards** (on-demand can't issue them, needs DNS). Additive — proxySnippet/redirectSnippet stay byte-identical when off (existing golden tests unchanged; +TestRender_OnDemandTLS + wildcard-skip). `db.Snapshot.OnDemandTLS` injected by caller; `plan.go` sets it per host. Setting `vhost_on_demand_tls` (default off) via `OnDemandTLSEnabled/SetOnDemandTLS`; injected in Reconcile + State(dry-run). UI: an "On-demand TLS: ON/OFF" switch in the vhosts reconcile-header (next to the live-reload arm), confirm modal spells out the prerequisite (global `on_demand_tls ask` block must exist first, else adapt fails → reload safely refused). Route `POST /post/vhost/on-demand-tls`; state field `onDemandTls`.
- **NOT done here (hand-managed / Owner, per rollout order):** item 2 global `on_demand_tls { ask http://127.0.0.1:<port>/internal/tls-ask … }` block in `/etc/caddy/Caddyfile`; item 4 remove dead ZeroSSL fallback (code 2977) — both live in the hand-managed Caddyfile (no ZeroSSL/issuer config anywhere in the panel — confirmed via grep). ⚠ Rollout a→b→c: (a) verify ask endpoint by curl; (b) global block + tls{on_demand} on ONE test vhost by hand; (c) flip the panel toggle → Reconcile for all hosts. Reload via `sudo caddy reload` only.
- Validation: `GOOS=linux go build ./...` 0; gofmt clean; render+reconcile tests pass (Windows); db test compiles for linux; client tsc 0; `npm run build` OK.

### 2026-07-26 - "IN USE" guard: flag load-bearing project dirs even when not deployed

- Goal (hub spec, safety follow-up to the compose view): "Not deployed" ≠ "unused." A near-miss today — the view showed `golang · 0 running · 1 not deployed`, reading as safe-to-delete, but `/home/server/htdocs/golang` is LIVE: the running `go-actions` container bind-mounts config/storage/production.env out of it, and the go GitHub Actions runner (`runner/bin/Runner.Listener`) runs from it. Deleting it would drop 4 tenants + go's deploys. Panel must SAY WHY a dir isn't dormant.
- Backend (`services/container_compose.go`): two cheap host passes, only run when there's ≥1 non-running row. (1) `runningBindMounts()` — `docker ps -q` then one `docker inspect` over the running set; `parseBindMounts` keeps `.Mounts[]` where Type=="bind". (2) `hostProcessExePaths()` — one `ps -eo args=`; `parseHostExePaths` takes absolute argv[0] (dedup). `computeInUse(dirs, binds, procs)` joins via `underDir` (POSIX prefix, sibling-safe: golang-app ≠ golang) → per in-use dir: which container mounts which sub-paths + which host procs. `ListWithCompose` attaches `InUse`/`InUseMounts`/`InUseProcs` to non-running rows (running = self-evidently in use). Uses clean-stdout `runStdout` (not the CombinedOutput runner) so stderr warnings don't corrupt the inspect JSON. New `MountUse{Container,Paths}` type on Container. Tests (`container_compose_test.go`): golang near-miss (flagged via both mounts+proc), runner/nocodb NOT flagged, parseBindMounts ignores volume/tmpfs, parseHostExePaths, underDir sibling-prefix.
- Frontend (`client/src/routes/containers/index.tsx`): amber "In use — live mounts" / "host process" badge (`InUseBadge`) in the State cell of not-deployed AND stopped rows; a self-evident inline reason line in the Container cell ("go-actions: config, storage, production.env · host process. Directory is not safe to delete.") with full detail on hover title. Not-deployed in-use rows get a faint amber row tint.
- Guard on destructive actions: per spec, deliberately did NOT add any Remove/Delete-project action — surfacing the state is the valuable half. If one is ever added it must hard-block/confirm on `inUse`.
- Validation: `GOOS=linux go build ./...` 0; gofmt clean; services test compiles for linux; client tsc 0; `npm run build` OK. (Tests RUN only on host via `make test` — Windows can't compile services/system.go's linux syscalls.)

### 2026-07-26 - Compose-project awareness on the Containers page ("not deployed" state)

- Goal (hub spec, Owner-driven): the Containers page listed only RUNTIME objects (`docker ps`), so a `docker compose down` (e.g. nocodb, to reclaim RAM) removed the container entirely and it VANISHED from the panel — no way to see or restart what was turned off. Make the page COMPOSE-PROJECT aware: list compose services joined against runtime, with a new **Not deployed** state (compose file defines it, no container exists) + a **Start** action. Mirrors the vhosts page's defined-vs-live model + summary line.
- Backend: `services/container_compose.go` (new) — `discoverComposeProjects()` scans compose files (`/home/server/htdocs/*` by default, override `COMPOSE_SCAN_DIRS`; unions live containers' `com.docker.compose.project.working_dir` labels so relocated projects aren't missed), cached 30s. `parseComposeServices()` — small structural parser for top-level `services:` keys (no YAML dep, no `docker compose config` shell-out; +tests `container_compose_test.go`). `ListWithCompose()` — real containers (Deployed=true) join on (workingDir, service); each compose service with no match becomes a synthetic Container{Deployed:false, State:"not_deployed"}; sorted by project then name (standalone last). `ComposeUp(workingDir, service)` — re-validates against discovered projects (can't drive `compose up` in an arbitrary dir), then `docker compose up -d <service>`. `containers.go`: added Project/Service/WorkingDir/Deployed to Container; parseDockerContainers extracts compose labels from the `Labels` field (`composeLabel`); both parsers set Deployed=true.
- Routes: `routes/post/containers/main.go` list Handler now calls `ListWithCompose()`; new `ComposeUpHandler` → `POST /post/containers/compose-up` (postOnly, root-only, returns {output,error}). Read-only discovery; only mutation is the explicit Start.
- Frontend (`client/src/routes/containers/index.tsx`): ContainerRecord gained project/service/workingDir/deployed. Summary line "N running · N stopped · N not deployed (· N standalone)". Rows grouped by project with a header row (project name + `N services · N running · N not deployed`); standalone/non-compose group last. Not-deployed rows are de-emphasized (muted, dashed marker, "Not deployed" pill, workingDir shown, no id/image/ports) with a single **Start** button → confirm modal (shows the `docker compose up -d <service>` command + dir) → `composeUp()`. "Show not deployed" checkbox (default on; only shown when any exist). No auto-start anywhere.
- Validation: `GOOS=linux go build ./...` 0; gofmt clean; services test compiles for linux (`go test -c`); parseComposeServices/composeLabel tests added; client tsc 0; `npm run build` OK. NOTE: `go test ./services` can't RUN on Windows (system.go uses linux `syscall.Statfs`) — must run on host via `make test`.

### 2026-07-24 - Edge disable/enable (suppress) for tenant + redirect + system vhosts

- Goal (Owner): a "disable/enable this vhost at Caddy whenever" control — the hostname terminates at Caddy, and server-panel (sole Caddy renderer) simply stops/starts serving it. Owner reasoned it's a Caddy-layer toggle, NOT a stack-DB write — correct, so no hub sign-off needed. Generalized across tenant (website_hosts), redirect (platform_redirect_hosts), AND system (platform_hosts) vhosts.
- Mechanism: PANEL-LOCAL suppress set (server-panel's own SQLite, new `vhost_suppressed_hosts` table) — never writes any DB row (stack-owned website_hosts stay untouched; panel-owned rows keep their is_active). Host-based + table-agnostic: `db.Snapshot.SuppressedHosts`; `BuildPlan` treats a suppressed host like a disabled row (don't render + remove its live vhost through the SAME safe reload path — first-pass suppression, drop-guard allows the removal as intentional, backup, adapt-validate). Re-enable = remove from set → next reconcile re-renders.
- Files changed: `services/caddy/db/db.go` (Snapshot.SuppressedHosts), `services/caddy/reconcile/plan.go` (skip suppressed hosts), `services/caddy/reconcile/engine.go` DryRun (suppressed tenants → stable "disabled" HostRow, not transient will_remove; suppressed files skipped from will_remove). `services/settings.go` (table) + `services/vhost_headers_store.go` (`SuppressedHosts`/`SetHostSuppressed`). `services/caddy_engine.go` — `SuppressHost(ctx, host, suppressed)` (gated, protected-domain-refused, then Reconcile); inject SuppressedHosts into Reconcile + State; `ManageRow.Suppressed` populated for system + redirect rows. `routes/post/vhost/main.go` `SuppressHandler` + `POST /post/vhost/suppress`. Frontend: shared `suppressHost()` + StatusChip "disabled·edge"; a Power/PowerOff disable/enable toggle in TenantView (Manage col; alongside Manage-in-stack), RedirectsView (distinct from soft-delete), SystemView (served = Active AND not suppressed; Pin hidden while suppressed). `index.tsx` passes onSaved to TenantView.
- Important decisions: suppress is orthogonal to is_active — for panel-owned hosts, is_active/edit = DB config, suppress = quick edge on/off (row kept). Protected domains (cp/app) refused. Gated by live-reconcile like every write. STILL TODO (Owner also asked): a details modal for tenant/redirect (read-only for stack-owned tenants, editable for redirects) showing the rendered vhost — not built yet this pass.
- Validation: `GOOS=linux CGO_ENABLED=0 go build ./...` 0; `go test ./services/caddy/reconcile/` pass; `gofmt` clean; `tsc --noEmit` 0; `npm run build` OK.
