...
- ⚠ STILL UNVERIFIED: the SPOF acceptance test has NOT produced a real result — the Owner pasted the curl loop and the restart into the SAME terminal, so the loop blocked and the restart never ran. Needs TWO terminals. Confirm before moving more hosts to cf_origin.
